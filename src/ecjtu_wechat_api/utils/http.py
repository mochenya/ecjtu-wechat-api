...
async def get_page(url: str, params: dict | None = None, timeout: int = 10) -> str:
    ...
    """
    async with httpx.AsyncClient(headers=settings.DEFAULT_HEADERS, timeout=timeout) as client:
        try:
            response = await client.get(url, params=params)
            response.encoding = "utf-8"

            if response.status_code != 200:
                logger.error(f"教务系统返回非 200 状态码: {response.status_code}, URL: {url}")
            ...
            raise EducationSystemError(message=f"网络请求失败: {str(e)}")
